    ...
    <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] bg-white border-t border-gray-100 z-50">
      <div className="flex items-center justify-around px-2 py-2">
        {tabs.map(tab => {
          ...
          return (
            <Link key={tab.href} href={tab.href} className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-colors ${isActive ? 'bg-blue-50' : ''}`}>
              <span className="text-lg">{isActive ? tab.activeIcon : tab.icon}</span>
              <span className={`text-[11px] ${isActive ? 'font-semibold text-blue-600' : 'font-medium text-gray-500'}`}>{tab.label}</span>
            </Link>
          );
        })}
      </div>
